#!/usr/bin/env python3
"""Score Yes versus No for binary camera questions over fixed image sequences."""
...
from scripts.camera_binary_vqa.runtime import answer_token_ids, yes_minus_no_score
            ...
            for index, sample in enumerate(rows):
                ...
                if not math.isfinite(score):
                    raise FloatingPointError(
                        f"non-finite score for {condition_name}:{sample.get('sample_id')}"
                    )
                result = {
                ...
                written += 1
                if written % 40 == 0:
                    print(f"rank={rank} scores={written}", flush=True)
