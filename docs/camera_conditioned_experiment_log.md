...
| 2026-07-13 | 有序抽帧二元相机辅助与检测回放联合训练三分支验证 | 三分支训练完成；相机能力子门通过；RL 前置门 borderline；待三分支评测 | 在相同有序抽帧输入和检测回放下，正确二元相机监督是否比逐条翻转监督及仅检测对照学到视觉相关能力，同时保留检测接口 | 三个分支均完成 3490 步并产出 final adapter 与 5 个 checkpoints；正确模型相机门通过，RL 仅允许短程；下一步只补测仅检测回放模型并汇总三分支 |
...
- 日期：2026-07-13。
- 状态：`三个分支训练完成；双分支 held-out 相机门通过；仅检测回放分支待相机与检测评测`。
...
| Final step | 3490 |
| Final loss / 完整训练耗时 | 待补充 |

结论标记：训练完成性`通过`，方法效果`结论不足`。第三个控制分支已具备评测条件，但尚未提供相机指标或 Real/Fake 检测指标；不能仅根据权重完整推断它优于或劣于相机辅助分支。
