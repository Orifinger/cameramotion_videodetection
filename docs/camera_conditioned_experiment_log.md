...
| 2026-07-14 | 完整 DataB 检测模型的 Skyra 风格 GRPO 奖励动力学诊断 | 准备执行；运行链路已通过 | 从既有检测 checkpoint 出发，比较论文式奖励及其问题消融如何改变正确率、真假偏置、证据计数和组内奖励方差 | 先运行 1 步端到端 GRPO；本实验用于学习训练曲线和奖励行为，不作为相机方法或泛化提升证据 |
...
- 日期：2026-07-14。
- 状态：`准备执行；16 图像 vLLM rollout 运行链路已通过，待 1 步 verl 端到端门`。
...
立即下一步：先执行数据构建、verl 兼容补丁和真实 `RLHFDataset` 预检；全部通过后只跑论文式奖励的 1 步端到端门。拿到 1 步 rollout、汇总 JSON 和日志后再决定是否并行启动 40 步奖励消融，避免把基础设施错误误判成奖励设计现象。
