...
| 2026-07-19 | DataB 到 ViF-Bench 的相机条件化几何残差最小验证 | 未通过；当前二维几何补偿路线停止 | 不使用 DataA、CoT 或相机文本，检验正确相机几何补偿后的残差是否稳定优于原始运动和错配几何控制 | 特征覆盖 100%；正确几何残差 ViF AUROC 56.88%，低于原始光流 59.13% 和错配几何 58.35%，全部预注册主门失败 |
...
- 服务器主摘要：`/tmp/1res/camera_geometric_residual_gate/v1/eval/camera_geometric_residual_gate_summary.json`。
- NAS 持久化目录：`/input/workflow_58770161/workspace/test/cameramotion_det/res/camera_geometric_residual_gate/v1/eval/`。
- 用户回传结果包：`E:/newgaibeishi/eval (5).zip`。

DataB 5639 个唯一视频和 ViF-Bench 3160 个视频的特征覆盖率均为 100%。ViF-Bench 含 2995 个 Fake 和 165 个 Real，因此主要读取 AUROC 与 Balanced ACC，不把受类别比例影响较大的普通 ACC、AP 或 Fake F1 作为本门依据。

| 冻结特征探针 | DataB val AUROC | DataB val Balanced ACC | ViF AUROC | ViF Balanced ACC | ViF motion-macro Balanced ACC |
|---|---:|---:|---:|---:|---:|
| DINO 外观 | 70.53% | 65.59% | 56.20% | 53.60% | 54.09% |
| 外观 + 原始 RAFT 光流 | 76.77% | 70.53% | 59.13% | 58.51% | 57.71% |
| 外观 + 正确几何残差 | 75.59% | 70.49% | 56.88% | 55.21% | 53.88% |
| 外观 + 时间错位几何残差 | 77.16% | 70.81% | 58.35% | 55.44% | 54.99% |

| 正确几何残差相对控制 | AUROC 差值 | Balanced ACC 差值 | motion-macro 差值 | AUROC paired bootstrap 95% CI |
|---|---:|---:|---:|---:|
| 相对原始光流 | -2.25 点 | -3.29 点 | -3.83 点 | [-4.73, +0.01] 点 |
| 相对时间错位几何 | -1.47 点 | -0.23 点 | -1.11 点 | [-3.76, +0.66] 点 |

补充审计：

- 正确几何残差在静止/无运动桶的 Balanced ACC 为 55.80%，比原始光流的 58.85% 低 3.05 点，未满足“下降不超过 1 点”的门槛。
- 原始光流相对纯外观的 ViF AUROC 高 2.93 点、Balanced ACC 高 4.91 点，说明普通 motion feature 在本协议中存在信号；但该比较不是正确相机补偿的证据，也可能包含已审计到的数据集运动偏置。
- `source_wins_when_supported=true` 不具有证据意义：当前 `source_name` 把 19 个生成器各自划成纯 Fake 组、把真实视频划成单独纯 Real 组，因此各来源 AUROC 均不可计算，实际 `supported_sources=0`。该检查是空真值，但其余七个预注册主检查均失败，不影响总判定。

结论标记：`未通过`。本轮直接否定了“在相同 16 帧上用 RAFT 拟合二维 homography/fundamental geometry，并把正确补偿残差作为冻结特征追加到检测器”能够优于原始运动的假设。它不否定所有 3D pose/depth 方法，但结合此前 DataA 上的补偿负结果，当前二维 RAFT/DINO 几何补偿路线不再进入 Qwen3-VL projector、SFT 或 RL。

立即下一步：停止当前几何残差融合实现，保留原始光流优于纯外观这一附带观察作为后续方法筛选依据；在确定新的相机—检测耦合机制前，不继续增加二维补偿模型或调参。
