...
| 2026-07-13 | 有序抽帧二元相机辅助与检测回放联合训练三分支验证 | 相机能力子门通过；生成式 RL 前置门未通过；检测对照待训 | 在相同有序抽帧输入和检测回放下，正确二元相机监督是否比逐条翻转监督及仅检测对照学到视觉相关能力，同时保留检测接口 | 正确模型 matched Macro AP 86.28%、Balanced ACC 74.44%，建立视觉依赖相机能力；但 900×8 次采样没有一个严格 Yes/No，当前生成接口不能直接进入 GRPO，待审计原始回复 |
