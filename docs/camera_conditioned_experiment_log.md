...
| 2026-07-17 | 二路检测专家交叉离线诊断 | 代码与测试通过，待服务器执行 | 在不训练和不重新推理的情况下，分别比较两个专家在静止/有运动路由子集上的表现，定位错误路由胜出的原因 | 只用于区分专家全局强弱、训练分布不平衡、Router 跨域问题或专家语义反转；使用已查看的 ViF 标签，不能挽救失败方法 |
...
- 输出：`/tmp/1res/camera_binary_route_gate/v1/diagnostics/expert_crossover.json` 与 `expert_crossover_compact.json`。
