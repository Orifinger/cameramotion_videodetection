...
| 2026-07-20 | 最终真假监督下的连续相机交互判别门 | 代码已就绪，待服务器执行 | 直接以 DataB Real/Fake 监督训练相机-证据交互分类器，检验连续相机几何能否带来外部检测增量 | 复用已审核 RAFT/DINO 特征；先在 ViF-Bench 比较 matched、等容量 zero-camera、shuffled-camera 与 camera-only |
...
服务器 A 复用本地 DataB 特征完成预检、短 smoke、三条件三 seed 训练及 DataB validation 校准；模型包写入共享 NAS。服务器 B 在模型包出现后复用本地 ViF 特征运行四条件外部评测。通过后才进入冻结 Qwen 检测分数融合和 GenBuster benchmark；未通过则停止把 camera 作为论文主贡献，不继续 camera 文本、camera RL 或路由试参。
