...
| 2026-07-14 | 完整 DataB 检测模型的 Skyra 风格 GRPO 奖励动力学诊断 | 可评测复跑训练及合并完成，待固定评测 | 从既有检测 checkpoint 出发，比较论文式奖励及其问题消融如何改变正确率、真假偏置、证据计数和组内奖励方差 | step 50/100 均已从 16 个 FSDP 分片合并为约 17 GB 的 HF 模型；先做 tokenizer 等价审计，再比较固定 DataB 策略漂移诊断和同协议 VIF-Bench |
...
结论标记：`通过（checkpoint 保存与 FSDP 合并）`；`结论不足（检测效果）`。当前只证明两个训练时点已经转成可加载的 HF 目录，不证明 step 50 或 step 100 优于起始模型。立即下一步是 tokenizer/processor 等价审计；通过后先跑固定 512 条 DataB 策略漂移诊断，只有输出协议和分类指标完整，才进入同协议 VIF-Bench。
