...
| 2026-07-14 | 完整 DataB 检测模型的 Skyra 风格 GRPO 奖励动力学诊断 | 完整 VIF-Bench 未通过 | 从既有检测 checkpoint 出发，比较论文式奖励及其问题消融如何改变正确率、真假偏置、证据计数和组内奖励方差 | step 50/100 均提高 Fake Recall，但因 Real Recall 更大幅下降而损失 Balanced ACC 和 Fake F1；当前论文式奖励配方不继续延长 |
