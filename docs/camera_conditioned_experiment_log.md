...
| 2026-07-11 | 相机运动前置感知强化学习最小验证 | 训练待执行 | 不向检测模型提供外部 camera 文本，先奖励模型从视频预测相机运动，再检验该能力是否迁移到检测 | P0 工具链与本地审计已通过；待服务器预检 ms-swift 和显式 split 后运行两个严格对照短模型 |
...
当前处于“工具链就绪、训练待执行”。第一轮不使用 DataB 伪 camera labels，不实现显式光流或 motion-token adapter。服务器上的 DataA train split 文件名和 ms-swift 版本仍需预检。
...
在服务器运行数据构建与图片路径检查，并提供 `swift --version`、`swift rlhf --help` 和构建 summary。确认 CLI 与 16 卡 rollout 参数后，编写并运行两条严格对照的 P1 smoke/full 启动脚本。
