...
| 2026-07-14 | 完整 DataB 检测模型的 Skyra 风格 GRPO 奖励动力学诊断 | 已完成；训练稳定，性能结论待固定验证 | 从既有检测 checkpoint 出发，比较论文式奖励及其问题消融如何改变正确率、真假偏置、证据计数和组内奖励方差 | 100 步无 Fake 单边塌缩或数值发散；奖励增长约 56% 来自论文预期的证据计数项，后 10 步零方差组为 65.63%；因未保存 checkpoint、未做固定验证，不能判定训练效果好坏 |
