...
|---|---|---|---|---|
| 2026-07-22 | 原生尺度 DINO 时序取证专家与强 Qwen 互补性两层验证 | 第一层通过，第二层未通过 | 先验证正确帧序是否产生可泛化的 Real/Fake 取证增益，再验证该专家能否以预注册固定融合改善强 Qwen | 帧序信号成立；固定融合仅改善排序、没有改善 Real/Fake 硬判定，暂不运行 GenBuster Closed Benchmark |
...
## 18. 连续相机条件下的时序正常性专家验证（CTNE Gate 1）

### 这个实验测什么

验证连续相机几何能否解释真实视频时序证据的一部分合法变化：只用真实训练视频学习 `p(Y | C, Real)`，再检查匹配当前视频的连续相机条件是否在外部 AIGC 视频检测上稳定优于同容量零条件模型和样本间打乱条件。它是 camera 是否对最终 Real/Fake 有独立增量的第一道方法门，不训练或评测 Qwen3-VL 检测答案。

### 日期、状态与模型谱系

- 日期：2026-07-20。
- 状态：`代码已就绪，待服务器执行`。
- 相机与时序特征：冻结的 TorchVision RAFT-Large 和 DINOv2-Small，不更新权重。
- RAFT 权重：`/home/admin/raft_large_C_T_SKHT_V2-ff5fadd5.pth`。
- DINOv2 权重：`/home/admin/dinov2-small`。
- 正常性模型：从头训练的 conditional masked autoregressive flow；3 个固定 seed，每个 seed 同时训练 matched 和同参数量 unconditional 两个模型。
- Qwen3-VL、CameraBench caption、camera labels 和旧 detection checkpoint 都不作为 Gate 1 模型输入；camera labels 只保留为语义审计字段。
- 执行说明：`scripts/camera_ctne_gate1/README.md`。

### 数据与准确路径

- DataB detection JSON：`/input/workflow_58770161/workspace/test/camb/camerabenchdataB-main/detection/v4vif_2766busterall_trainall.json`。去除完全重复帧序列后按来源、真假、连续运动桶和实际帧数桶做确定性 train/validation 划分；flow 与 scaler/PCA 只使用 train 中的 Real 视频。
- DataB camera sidecar：`/input/workflow_58770161/workspace/test/camb/camerabenchdataB-main/datab_cameramotion_labels_final/datab_cameramotion_labels_v2.jsonl`，只做覆盖与语义审计，不进入模型。
- ViF-Bench 分片索引：`/input/workflow_58770161/workspace/test/cameramotion_det/eval/v4train-main/test_index_splits/splits_16`。
- ViF-Bench camera sidecar：`/input/workflow_58770161/workspace/test/camb/camerabench_outputs/vifbench_cameramotion_labels_v2/datab_cameramotion_labels_v2.jsonl`，同样不进入模型。
- GenBuster-200K：只使用 Hugging Face 数据集中的 `benchmark` 集；服务器实际帧根目录为 `待补充`。不得把 DataB 已包含的 GenBuster train/test 子集当成外部 benchmark。
- DataA 不参与 Gate 1 主训练或主验收；Gate 1 通过后才用于局部编辑定位和解释诊断。

### 单一改变因素和控制

每个视频按 JSON 或 benchmark 目录中实际存在的完整有序帧构造 `T_i=n_i-1` 条相邻 transition。`C` 是 RAFT 全局几何的连续平移、旋转、尺度、shear、透视、inlier 与误差；`Y` 保留 DINOv2 一阶/二阶帧差、patch 变化和未补偿 RAFT flow 统计。这里不再做 `observed_flow - camera_flow` 的硬相减。

| 条件 | 唯一变化 | 作用 |
|---|---|---|
| 正确连续相机条件 | matched flow 接收当前视频逐 transition 的 `C` | 主方法 |
| 同容量无条件 | 相同架构和每 seed 相同初始化，但 `C` 全置零 | 判断收益是否只是普通时序正常性 |
| 样本间打乱相机条件 | 不训练新模型，只在评测时换入非自身 donor 的 `C` 并插值到当前长度 | 判断 matched camera-content 配对是否真的被利用 |
| camera-only 线性探针 | 只看视频级 `C` 统计 | 检查生成器、来源或运动边际捷径 |
| 历史 raw/residual 探针 | 复用 2026-07-12 已记录结果 | 明确新方法不是重复调旧 residual |

### 主要设置与输出

- 正式默认 `MAX_FRAMES=0`，不假定或静默截成 16 帧。仅用户显式设正数时才做保留首尾的均匀限帧，并把原帧数、使用帧数和索引写入元数据。
- `n_i<3` 标为 `ctne_unavailable` 并报告覆盖率；不补零伪造二阶证据。RAFT/DINO 按最多 32 帧的小块带一帧重叠处理，每条相邻 transition 只计算一次。
- scaler、PCA 和 flow 只在 DataB train Real 上拟合。训练损失先对每个视频的 transition 平均，因此每个视频总权重都是 1，不让帧数较多样本占更大权重。
- matched/unconditional 使用 64 维 PCA 证据、4 个 MAF transform、hidden 128、AdamW `2e-4`、最多 60 epochs、patience 8、3 seeds。两者每个 seed 具有相同参数量和初始权重。
- 视频异常分数预先固定为 transition NLL 的 `0.5 * mean + 0.5 * q90`。分类阈值只在 DataB validation 固定，ViF-Bench 与 GenBuster benchmark 禁止调参。
- 大型 transition NPZ 放 `/tmp/1res/camera_ctne_gate1/v1`；manifest、审计、feature index、校准与指标放 `${PROJECT_ROOT}/res/camera_ctne_gate1/v1`。通过审计且后续复用的大特征才上传 OSS 的 `camera_flow/ctne_gate1_v1/`。

### 验收标准

- 特征审计覆盖率至少 98%，所有归档维度一致、数值有限；`MAX_FRAMES=0` 时每条归档必须使用 manifest 的全部帧。
- ViF-Bench 与 GenBuster benchmark 中，至少一个数据集的 pooled AUROC 或跨生成器 Macro Balanced ACC 相对 unconditional 提高至少 1.0 点，另一指标不得下降超过 0.5 点；另一 benchmark 的两项主指标也都不得下降超过 0.5 点。
- matched 必须优于 shuffled；至少一个主指标的 paired bootstrap 95% CI 下界大于 0，且该结论在两套外部 benchmark 上成立。
- camera-only 外部 AUROC 不高于 0.65；否则 camera marginal/来源捷径构成混杂，不能把 matched 收益归因于 camera-conditioned normality。
- 至少两个连续 motion bucket 和过半数有充分样本的 fake generator 上，matched 相对 unconditional 的方向一致；完整报告 3 seeds，不只保留最好 seed。

### ViF-Bench 正式结果（2026-07-20）

- 结果来源：`/input/workflow_58770161/workspace/test/cameramotion_det/res/camera_ctne_gate1/v1/eval/vifbench/ctne_gate1_summary.json`。
- 评测规模：3160 条，其中 Fake 2995、Real 165；阈值只由 DataB validation 固定，ViF-Bench 未调参。
- 状态：`未通过`。

| 条件 | AUROC | Balanced ACC | Real Recall | Fake Recall | 跨生成器 Macro Balanced ACC | 预测 Fake 比例 |
|---|---:|---:|---:|---:|---:|---:|
| 正确连续相机条件（matched） | 40.91% | 50.14% | 0.61% | 99.67% | 50.14% | 99.65% |
| 同容量零条件（unconditional） | 40.92% | 50.17% | 0.61% | 99.73% | 50.18% | 99.72% |
| 样本间打乱相机（shuffled） | 39.67% | 49.48% | 98.79% | 0.17% | 49.48% | 0.22% |
| 仅相机统计（camera-only） | 59.31% | 56.09% | 46.06% | 66.11% | 56.26% | 65.47% |

matched 相对 unconditional 的 AUROC 差为 `-0.000093`（-0.009 点），跨生成器 Macro Balanced ACC 差为 `-0.000319`（-0.032 点）；paired bootstrap AUROC 差的 95% CI 为 `[-0.00462, 0.00354]`。matched 虽比 shuffled 的点估计高 1.25 个 AUROC 点，但其 95% CI 为 `[-0.01225, 0.04101]`，仍跨 0。三个 motion bucket 和 19 个生成器均没有出现稳定正向增量。

高达 94% 以上的普通 Accuracy 和 Fake F1 来自测试集严重偏向 Fake 且模型几乎全判 Fake，不能解释为检测有效；阈值无关的 AUROC 也低于 0.5，说明 DataB-real 正常性分数向 ViF-Bench 迁移时存在严重域偏移。camera-only AUROC 59.31% 未触发 65% 捷径上限，但它反而明显高于两个 flow，进一步说明当前条件密度模型没有把连续相机信息转化为异常检测增量。

本结果证明的是当前 `RAFT/DINO + p(Y|C,Real)` 配方在 ViF-Bench 上失败，不证明任何 camera 信息对所有 AIGC 检测方法都无用。由于预注册条件要求 matched 在两套外部集上都显著优于 shuffled，而 ViF-Bench 已不满足，CTNE Gate 1 的 camera 主张无法再由 GenBuster 结果挽救；因此不为挽救该门继续上传大特征或启动 Gate 2。GenBuster 只在另有独立诊断问题时再运行。

### 泄漏、分布差异与结论边界

- DataB 是当前训练域且旧 Qwen checkpoint 已见过它；其 validation 只用于正常性模型 early stopping、阈值和校准，不称为外部结果。
- 构建外部 manifest 后必须按路径和 `source/label/identity-tail` 审计与 DataB 的重叠。发现重叠时先剔除或修正数据合同，不继续计算漂亮但泄漏的指标。
- ViF-Bench 已被项目多次查看，属于外部开发 benchmark；GenBuster benchmark 是第二个外部验证。两者都不能反向选择 PCA、flow 架构、聚合权重或阈值。
- Gate 1 通过只证明 camera 对独立时序专家有增量，不证明 Qwen 最终 Real/Fake 已提高。只有两套外部结果合并门通过后，才进入冻结原 detection checkpoint 的分数融合 Gate 2。
- Gate 1 未通过则停止 camera 作为论文主贡献，不继续做 pose token、caption SFT 或 camera RL；可保留 unconditional temporal expert 转向不声称 camera 的通用时序检测路线。

### 立即下一步

服务器 A 先执行 DataB build，使持久化 manifest 可供两台机器共享；随后服务器 A 用 16 GPU 提取 DataB、训练 3×2 个 flow 并生成 DataB validation 校准包，服务器 B 并行提取 ViF-Bench。确认 GenBuster benchmark 的实际帧根目录后，在空闲服务器提取第二外部集。先查看 feature audit，再分别生成两个 `ctne_gate1_summary.json`，最后合并为 `ctne_gate1_final_decision.json`；在结果出现前不启动 Gate 2。

## 19. 最终真假监督下的连续相机交互判别门

### 这个实验测什么

直接用 DataB 的 Real/Fake 标签监督一个变长时序分类器，检验连续相机几何 `C_t` 与局部/时序证据 `Y_t` 在同一最终检测损失中交互后，是否比完全相同容量但不使用相机信息的模型更能泛化到 ViF-Bench。它专门修正上一轮 CTNE 只拟合 `p(Y|C, Real)`、没有直接建立 camera 与真假终点联系的问题。

### 日期、状态与模型谱系

- 日期：2026-07-20。
- 状态：`未通过；ViF-Bench 正式门失败`。
- 起点：不使用 Qwen3-VL checkpoint；复用 CTNE Gate 1 已冻结并审核通过的 RAFT-Large、DINOv2-Small 连续特征。
- 主模型：从头训练的轻量 Camera-FiLM 变长时序分类器，直接优化 Real/Fake BCE。
- 三个训练条件各使用 seeds `13/37/73`，每个 seed 内参数量、初始化、样本、优化器和训练预算一致。
- 执行说明：`scripts/camera_discriminative_gate/README.md`。

### 数据与路径

- DataB 特征索引：`/input/workflow_58770161/workspace/test/cameramotion_det/res/camera_ctne_gate1/v1/features/datab_feature_index.jsonl`。
- DataB 大特征：服务器 A 本地 `/tmp/1res/camera_ctne_gate1/v1/datab_features/`；索引内保留真实变长 transition，不假设 16 帧。
- ViF-Bench 特征索引：`/input/workflow_58770161/workspace/test/cameramotion_det/res/camera_ctne_gate1/v1/features/vifbench_feature_index.jsonl`。
- ViF-Bench 大特征：服务器 B 本地 `/tmp/1res/camera_ctne_gate1/v1/vifbench_features/`。
- scaler/PCA 只在 DataB train 的 Real+Fake 视频上拟合；每个视频仅在预处理统计拟合时等权重采样 16 个 transition，分类训练和评测仍使用每条视频完整的实际变长序列。
- DataB validation 只负责 early stopping 与阈值固定，不能作为外部测试；ViF-Bench 是本轮外部开发 benchmark，禁止在其上选择阈值或改超参数。

### 单一改变量与控制条件

| 条件 | 训练/评测输入 | 作用 |
|---|---|---|
| 正确连续相机交互（matched） | `Y_t` 与当前视频匹配的 `C_t`，Camera-FiLM 后预测 Real/Fake | 主方法 |
| 等容量零相机条件（zero-camera） | 同一 `Y_t`，但 `C_t` 全为零 | 隔离相机的增量，排除额外模型容量与训练预算 |
| 样本间打乱相机（shuffled-camera） | matched 模型不重训，只在评测时换入其他样本的 `C_t` | 检查模型是否真正依赖 camera-content 配对 |
| 仅相机条件（camera-only） | `Y_t` 全为零，只保留 `C_t` | 检查数据源、生成器或运动分布捷径 |

四个条件都不输入 camera label、caption 或 camera 文本。主模型先编码 `Y_t`，再由连续 `C_t` 产生 FiLM 的 `gamma/beta`，使用有 padding mask 的 attention、mean、max pooling 得到视频级 Real/Fake logit。最终损失只有可验证的二分类 BCE，不使用相机标签奖励，也不启动 RL。

### 重要设置与验收标准

- 证据 PCA 维度 64，hidden 128，AdamW `3e-4`，batch 128，最多 50 epochs，patience 8，三个固定 seeds。
- `matched` 相对 `zero-camera` 在 ViF-Bench pooled AUROC 或跨生成器 Macro Balanced ACC 至少提高 1.0 点，另一项下降不超过 0.5 点。
- `matched` 必须优于 `shuffled-camera`，且至少一项 paired bootstrap 95% CI 下界大于 0。
- 至少两个 motion bucket 正向、过半受支持 fake generator 正向、至少 2/3 seeds 的 AUROC 正向。
- camera-only AUROC 不高于 0.65，且 matched 不低于 camera-only；否则把结果标为相机边际/数据源捷径，不能归因于相机-证据交互。

### 泄漏、分布差异与存储

旧 DataB detection checkpoint 已见过 DataB，但本实验完全不加载该 checkpoint，因此 DataB train/validation 对当前从头训练小分类器没有权重继承泄漏。DataB 与 ViF-Bench 的类别比例、生成器、分辨率和运动分布仍不同；这正是必须使用 AUROC、Balanced ACC、跨生成器宏平均及 camera-only 控制的原因。ViF-Bench 已被项目多次查看，只是外部开发 benchmark，不声称为最终盲测。

准备后的 packed NPZ 放在各服务器 `/tmp/1res/camera_discriminative_gate/v1/`，属于可重建中间文件。九个小模型、DataB calibration、CSV 和 JSON 正式结果放在 NAS 的 `res/camera_discriminative_gate/v1/`。已审核且本轮继续复用的 DataB/ViF 大特征需分别上传 OSS 的 `camera_flow/ctne_gate1_v1/datab_features/` 和 `camera_flow/ctne_gate1_v1/vifbench_features/`。

### 监督判别门 ViF-Bench 正式结果（2026-07-20）

- 结果来源：`/input/workflow_58770161/workspace/test/cameramotion_det/res/camera_discriminative_gate/v1/eval/vifbench/camera_discriminative_gate_summary.json`。
- 训练完整性：正式 `train/models` 下生成 9 个模型，覆盖三个条件与 seeds `13/37/73`；三个 seed 内初始权重与参数量审计均通过。评测覆盖 ViF-Bench 3160 条，并完成 2000 次 paired bootstrap。
- 状态：`未通过`。六项结构与方向检查中五项通过，但 matched 相对 shuffled-camera 的 paired bootstrap 置信区间跨 0，无法证明收益来自正确的 camera-content 配对。

| 条件 | AUROC | Balanced ACC | 跨生成器 Macro Balanced ACC | Real Recall | Fake Recall |
|---|---:|---:|---:|---:|---:|
| 正确连续相机交互（matched） | 62.63% | 59.04% | 59.15% | 66.06% | 52.02% |
| 等容量零相机条件（evidence-only） | 62.26% | 57.91% | 57.95% | 70.30% | 45.51% |
| 样本间打乱相机（shuffled-camera） | 62.37% | 58.50% | 58.58% | 68.48% | 48.51% |
| 仅相机条件（camera-only） | 59.45% | 57.79% | 57.97% | 46.06% | 69.52% |

matched 相对 evidence-only 的 AUROC 提高 `0.37` 点、跨生成器 Macro Balanced ACC 提高 `1.20` 点，但两者 95% CI 分别为 `[-1.76, 2.69]` 点和 `[-1.84, 4.24]` 点。matched 相对 shuffled-camera 只提高 `0.25` AUROC 点和 `0.57` Macro Balanced ACC 点，对应 95% CI 也均跨 0。三个 seed 的 AUROC 方向都为正，但只提高约 `0.04/0.27/0.53` 点；两个 motion bucket 和 14/19 个生成器方向为正。

这说明直接 Real/Fake 监督确实比上一轮仅学习真实分布的 CTNE 更接近检测终点，并出现小幅一致正趋势；但正确相机与打乱相机在统计上不可区分，当前证据不足以把提升归因于相机-时序证据交互。该门不支持继续把 camera 作为论文主贡献，也不支持据此启动 Qwen 融合或 camera RL。
...
- 日期：2026-07-20。
- 状态：未通过；本地完整审计已完成。
- Qwen 分支：Qwen3-VL-8B-Instruct 经完整 DataB detection SFT 得到的 checkpoint-2115，使用历史 83.96% 原始 ViF-Bench 协议的 3160 条逐样本预测，而不是 79.18% 严格提示词诊断预测。两者权重相同，区别是推理提示词协议。
...
- Qwen 预测来源：E:/newgaibeishi/v4vif_2766busterall_trainall-3vl8b-vifbench.zip 内的 Qwen3-VL-v4vif_2766busterall_trainall-vifbench.json；SHA256 为 70F2633841E72FD96D5A229BF9C6FD8AC2E084F6359B0D2EB9114A8568E972D0。
- 时序专家逐样本结果来源：E:/newgaibeishi/vifbench.zip 内的 camera_discriminative_gate_items.csv；SHA256 为 FEBF5C11A125B91C3C560F57DF02E6C2522BF4A47826246E4F52010502A86B9D。
- 正式小结果：docs/vifbench_residual_complementarity_20260720.json 与 docs/vifbench_residual_complementarity_20260720.md；完整逐样本 CSV 保留在本地审计输出目录，不提交仓库。
...
两套输入各有 3160 条且 ID 交集为 3160；Qwen 有 4 条无效格式，因此主分析覆盖 3156 条，覆盖率与格式有效率均为 99.87%，无标签冲突。原始历史“83.96%”是跨生成器平均口径；在排除 4 条无效预测后的逐样本审计中，Qwen 跨生成器 Macro Balanced ACC 为 83.91%，数值一致。

| 条件 | Balanced ACC | 跨生成器 Macro Balanced ACC | Fake F1 | AUROC |
|---|---:|---:|---:|---:|
| 原 DataB 检测 Qwen | 83.79% | 83.91% | 94.44% | 硬标签，无原始连续置信度 |
| 时序证据专家 | 57.92% | 57.96% | 61.88% | 62.26% |
| Qwen 硬标签 + 专家分数五折 OOF 融合 | 83.79% | 83.91% | 94.44% | 85.02% |

| 错误关系 | 样本数 | 占比 |
|---|---:|---:|
| 两者都正确 | 1356 | 42.97% |
| 仅时序专家正确，可潜在补救 | 122 | 3.87% |
| 仅 Qwen 正确，错误切换会造成伤害 | 1481 | 46.93% |
| 两者都错误 | 197 | 6.24% |

Qwen 共错 319 条，专家能判对其中 122 条，表面 rescue coverage 为 38.24%；但在两者意见不一致的 1603 条中，Qwen 正确 1481 条、专家正确仅 122 条。因此 93.76% 的 Oracle accuracy 依赖一个尚不存在的近乎完美路由器，不能当成可实现增益。

预注册的 0.5 阈值下，五折 OOF 融合没有翻转任何 Qwen 答案：Balanced ACC、Macro Balanced ACC 和 Fake F1 增量均为 0，分组 bootstrap 差异恒为 0，100 次打乱专家分数也产生相同硬判定。融合连续分数 AUROC 为 85.02%，说明专家分数在 Qwen 两个硬标签组内部提供了少量排序信息，但没有形成可用分类边界。

为排除固定 0.5 阈值过严，另做了明确标记为事后诊断的嵌套分组阈值校准：每个外层测试折的阈值只由内层训练折确定。它只额外纠正 3 条 Fake，Balanced ACC 从 83.787% 到 83.838%，跨生成器 Macro Balanced ACC 从 83.907% 到 83.955%，约提高 0.05 点，仍远低于预注册的 0.5 点门槛。

结论标记：未通过。当前 evidence-only 时序专家确有少量潜在补救样本，但其整体强度和“何时应该信任专家”的可辨识性不足，无法对较强 Qwen 检测器形成稳定的最终 Real/Fake 增量。结果否定当前专家融合配方，不是否定所有时序专家；尤其当前 Qwen 只有硬标签，不能检验使用原始 token logits 的最佳置信度融合上限。
### 泄漏、结论边界与立即下一步
...
历史强 Qwen 检测结果只有最终 Real/Fake 硬答案，上一门无法知道模型何时犹豫。本实验不重新生成或替换历史答案，而是在完全相同的 ViF-Bench 帧序列、system prompt、user prompt 和已生成 CoT 下，teacher-force 到 `<answer>` 之前，补算 Real/Fake 两个答案在 tokenizer 中首个分歧 token 的条件 logit。随后用原视频分组的五折 OOF 对比“仅 Qwen 置信度”“Qwen 加无相机时序证据”“Qwen 加正确相机交互专家”“Qwen 加打乱相机交互专家”和“Qwen 加仅相机专家”，回答缺少 Qwen 置信度是否是上一轮无法可靠路由的主要原因。

### 日期、状态与模型谱系

- 日期：2026-07-20。
- 状态：`未通过；当前 camera/RAFT-DINO 专家融合配方停止`。
...
- Real/Fake 必须在 tokenizer 中具有共同前缀和可识别的首个分歧 token；补算分数的 argmax 必须与历史贪心答案至少 99% 一致，否则说明提示词或 token 合约没有复现，停止解释融合结果。
...
结果来源：`/input/workflow_58770161/workspace/test/cameramotion_det/res/vifbench_qwen_confidence_fusion/v1/eval/vifbench_confidence_fusion_summary.json`。

| 条件 | 有效样本 | ACC | Balanced ACC | Fake F1 | 跨生成器 Macro Balanced ACC |
|---|---:|---:|---:|---:|---:|
| 历史 Qwen 硬预测 | 3156 | 89.89% | 83.79% | 94.44% | 83.91% |
| 仅 Qwen 置信度校准 | 3156 | 89.89% | 83.79% | 94.44% | 83.91% |
| Qwen + 无相机时序证据 | 3156 | 89.89% | 83.79% | 94.44% | 83.91% |
| Qwen + 正确相机交互 | 3156 | 89.86% | 83.77% | 94.42% | 83.89% |
| Qwen + 打乱相机交互 | 3156 | 89.86% | 83.77% | 94.42% | 83.89% |
| Qwen + 仅相机专家 | 3156 | 89.89% | 83.79% | 94.44% | 83.91% |

- 3160 条中 3156 条具有有效答案 token 合约，覆盖率 99.87%；补算 logit 的 argmax 对这 3156 条历史答案复现率为 100%，因此 Qwen 置信度前向合约通过。
- 无相机时序证据相对仅 Qwen 的跨生成器 Macro Balanced ACC 增量为 0，分组 bootstrap 95% CI 为 `[0, 0]`。
- 正确相机交互相对仅 Qwen 的点增量为 -0.016 个百分点，95% CI 为 `[-0.049, 0]` 个百分点；相对无相机时序证据同样略降。
- 正确相机交互与打乱相机交互的主指标、硬预测和 bootstrap 增量完全相同；虽然连续排序指标有波动，但打乱相机的 ROC-AUC 85.93% 还高于正确相机的 85.07%，不能归因于正确 camera-content 对应关系。

结论标记：`未通过`。本门证明缺少 Qwen 连续置信度不是上一轮融合失败的主要原因；在强 Qwen 已有置信度条件下，当前时序证据没有形成任何决策增量，正确相机交互也没有优于打乱对照。该结果不否定所有 camera 方法，但已充分否定继续调当前 RAFT/DINO 专家、硬路由或同特征融合超参数的必要性；下一步应按强 Qwen 的残余错误类型重新选择与其互补的专项证据，而不是继续扩展当前 camera 专家配方。
...
## 23. ViF-Bench 强检测模型残余错误结构审计

### 日期、状态与问题

- 日期：2026-07-20。
- 状态：`已完成；当前 camera/RAFT-DINO 推理融合路线闭环否定`。
- 问题：319 个强 Qwen 残余错误是否集中于可解释的数据或相机条件，现有专家为什么无法转化为最终 Real/Fake 增量，以及 camera 是否仍有新的低成本作用点。
- 本分析不训练或重新推理 Qwen；ViF-Bench 标签参与统计与 OOF 校准，因此只用于方向选择。

### 数据来源

- 融合逐样本结果：`/input/workflow_58770161/workspace/test/cameramotion_det/res/vifbench_qwen_confidence_fusion/v1/eval/vifbench_confidence_fusion_items.csv`。
- 专家逐样本结果：`/input/workflow_58770161/workspace/test/cameramotion_det/res/camera_discriminative_gate/v1/eval/vifbench/camera_discriminative_gate_items.csv`。
- 历史 Qwen CoT：`/input/workflow_58770161/workspace/test/test_selfcot/Skyra/res/v4vif_2766busterall_trainall/v4vif_2766busterall_trainall-3vl8b-vifbench/Qwen3-VL-v4vif_2766busterall_trainall-vifbench.json`。
- ViF-Bench camera labels/caption：`/input/workflow_58770161/workspace/test/camb/camerabench_outputs/vifbench_cameramotion_labels_v2/datab_cameramotion_labels_v2.jsonl`。
- 完整分析报告：`docs/vifbench_residual_error_analysis_20260720.md`。
...
| Real→Fake | 38 |
| Fake→Real | 281 |
| 六类 I2V/编辑模型占 Fake | 33.0% |
| 六类 I2V/编辑模型占 Fake→Real | 71.2% |
| Hunyuan-I2V + Wan-VACE 漏检 | 116/330 |
| matched 专家分歧中的救错比例 | 9.75% |
| camera-only 专家分歧中的救错比例 | 17.32% |
| 最低置信度 10% 对错误的覆盖 | 23.8% |
| margin + coarse camera 嵌套校准增量 | +0.032 点，95% CI 下界为 0 |
| margin + fine camera labels 增量 | -2.240 点 |

Fake 在复杂运动下的漏检率为 15.23%，高于 minor 的 8.33% 和 static 的 6.87%；Real 则相反，static 下误报率达到 31.67%。按 Fake 生成器分层后，fast、complex、unsteady 和 pan 仍与漏检相关，说明 camera motion 是检测难度轴。但它没有形成足够强的 sample-specific 纠错信号：粗粒度校准只救回 2/319，细粒度标签产生 19 次救错和 33 次误伤，且 matched 不能稳定超过 shuffled。

CoT 同样呈现类别模板化：38 个 Real→Fake 全部输出伪影标签，而 281 个 Fake→Real 中只有 1 个输出伪影标签；有同源 Real 的 Fake→Real CoT 与 Real CoT 相似度中位数为 0.151，正确 Fake 为 0.084。这支持模型在源内容保持型生成视频上直接进入 Real 验证模板，但不能替代视觉人工审计来确定真正缺失的取证线索。

### 结论与下一步

结论标记：`已完成，当前推理 camera 方法未通过`。结果排除了继续调当前 camera 文本、RAFT/DINO 相机交互、硬路由和置信度融合的必要性。更可信的问题定义是“高质量 I2V/编辑视频的源内容捷径与困难样本不足”，camera 目前只能作为训练时难度分层候选。
...
**状态：第一层时序因果能力门通过；第二层固定融合互补性门未通过。**
...
- 第一层：`/input/workflow_58770161/workspace/test/cameramotion_det/res/forensic_temporal_expert_gate/v1/eval/forensic_temporal_expert_gate1_summary.json`
- 第二层：`/input/workflow_58770161/workspace/test/cameramotion_det/res/forensic_temporal_expert_gate/v1/eval/forensic_temporal_expert_gate2_summary.json`
- 有效 ViF 特征为 `3160/3160`；第二层因 Qwen confidence 对齐缺 4 条，实际覆盖 `3156/3160=99.87%`。

#### 第一层：正确帧序是否产生独立取证信号

| 专家条件 | ROC-AUC | 跨生成器 Macro Balanced ACC | Balanced ACC | Real Recall |
|---|---:|---:|---:|---:|
| 静态集合专家 | 76.73% | 72.20% | 72.02% | 67.27% |
| 正确顺序专家 | 78.43% | 73.81% | 73.59% | 69.09% |
| 正确顺序专家 + 输入打乱 | 76.09% | 70.33% | 70.13% | 67.88% |
| 打乱监督专家 | 76.80% | 71.06% | 70.86% | 66.67% |

- 正确顺序相对静态集合：ROC-AUC `+1.70` 点，跨生成器 Macro Balanced ACC `+1.61` 点。
- 正确顺序相对打乱监督：ROC-AUC `+1.63` 点，跨生成器 Macro Balanced ACC `+2.75` 点。
- 把正确顺序专家的输入打乱后：ROC-AUC `-2.34` 点，跨生成器 Macro Balanced ACC `-3.48` 点。
- 正确顺序相对静态集合的 ROC-AUC bootstrap 95% CI 为 `[+0.54,+2.86]` 点，三个种子方向一致，且 Real Recall 没有下降。

**第一层结论：通过。**这证明 DataB 上训练的 DINO 时序专家在 ViF 开发集上确实利用了帧序，并获得静态内容及等容量打乱控制无法解释的跨域 Real/Fake 增量；它不等于最终 GenBuster Closed Benchmark 已提升。

#### 第二层：该时序信号能否用固定权重改善强 Qwen 硬判定

| 条件 | Balanced ACC | 跨生成器 Macro Balanced ACC | ROC-AUC | AP |
|---|---:|---:|---:|---:|
| 历史强 Qwen | 83.79% | 83.91% | 84.98% | 98.62% |
| Qwen + 正确顺序专家，固定 `beta=0.25` | 83.79% | 83.91% | 89.37% | 99.23% |
| Qwen + 静态集合专家 | 83.79% | 83.91% | 88.61% | 99.16% |
| Qwen + 正确顺序专家的打乱输入 | 83.79% | 83.91% | 88.50% | 99.17% |
| Qwen + 打乱监督专家 | 83.79% | 83.91% | 88.76% | 99.10% |

- 固定融合没有改变 0.5 阈值下的任何主硬分类指标，bootstrap 增益及其置信区间均为 0。
- 排序指标出现明显改善：正确顺序融合相对 Qwen 的 ROC-AUC 为 `+4.39` 点，而且高于三个控制；这只能说明分数中存在互补排序信息，不能替代 Real/Fake 硬判定增益。
- 错误象限为：共同正确 2292、仅时序专家正确 157、仅 Qwen 正确 545、共同错误 162。专家存在可救错误，但明显弱于 Qwen，固定线性权重没有把这些互补样本转化为正确翻转。

**第二层结论：未通过。**预注册固定融合没有满足最终 Real/Fake 主指标和正 bootstrap 下界，因此现在不能运行 GenBuster Closed Benchmark，也不能声称完整方法成立。

### 追加诊断：分数尺度、阈值与标量路由（2026-07-23）

分析来源：`E:/newgaibeishi/forensic_temporal_expert_gate2_items.csv`，共 3156 条。本诊断读取了 ViF 标签，因此所有最优值仅是失败原因分析，不能作为正式测试结果或最终方法调参依据。

| 诊断条件 | Balanced ACC | 跨生成器 Macro Balanced ACC | 相对原 Qwen | 硬答案翻转数 |
|---|---:|---:|---:|---:|
| 原 Qwen，阈值 0.5 | 83.79% | 83.91% | - | 0 |
| 预注册 `beta=0.25`，阈值 0.5 | 83.79% | 83.91% | 0.00 点 | 0 |
| 同集最优全局 `beta=1.898`，阈值 0.5 | 83.84% | 83.96% | +0.05 点 | 3 |
| 同集联合优化 beta 与阈值的 oracle 上限 | 84.11% | 84.24% | +0.33/+0.34 点 | 57 |
| 5 折拟合全局 beta 与阈值 | 82.22% | 82.40% | -0.90/-0.84 点 | 待折汇总 |
| 5 折仅用两路标量分数训练条件路由 | 83.21% | 83.34% | -0.57/-0.57 点 | 4 |

- Qwen 绝对 logit 的最小值为 `8.75`、中位数为 `20.50`；时序专家绝对 logit 的中位数为 `5.23`、最大值为 `10.34`。因此 `beta=0.25` 的最大专家修正约为 `2.58`，数学上不足以跨过 Qwen 的最小决策边界，零翻转是必然结果。
- 157 条“仅专家正确”样本所需 beta 的中位数为 `5.35`，而 545 条“仅 Qwen 正确”样本被破坏所需 beta 的中位数为 `9.75`；两类跨界区间严重重叠，全局 beta 无法稳定选择性救错。
- 同集 oracle 也只能带来约 0.33 点 Balanced ACC，且 5 折全局校准和标量路由均退化，说明失败不只是 `beta=0.25` 取值过小，而是标量后融合缺少样本级可辨识信息。

**追加诊断结论：未通过。**停止继续搜索 beta、阈值、Qwen 置信度路由或仅依赖两路标量分数的后融合；Gate 1 的时序特征结论仍然成立，下一步只能验证训练阶段的特征/token 级耦合。
...
- 当前需要继续复用的模型 bundle 已在 NAS；现有 ViF NPZ 仍是诊断性临时产物，不要求上传 OSS。Gate 2 未通过前不固化完整特征。
- 分数尺度与决策边界诊断已经完成并未通过；不再搜索 beta、阈值或标量路由，也不运行 GenBuster Closed Benchmark。
- 立即下一步：登记并验证训练阶段的特征/token 级耦合最小实验。该实验必须直接优化 Real/Fake，使用正确顺序、静态集合和打乱顺序等容量控制；只有 ViF 硬判定指标形成稳定增益后才冻结方案。
