...
| 2026-07-12 | 相机补偿局部感知轨迹最小验证 | 全量提取与有效监督审计通过，待三探针对照 | 在相同密集原视频帧和局部 mask 监督下，显式相机补偿是否稳定优于未补偿局部轨迹 | 有效正监督 1068/1080；三个探针统一训练 749 case、主测试统一 319 case，完整 321 test 仅作敏感性结果 |
...
- 日期：2026-07-12。
- 状态：数据和权重预检、来源×运动分层 smoke、人工可视化、1080-case 文件与相机几何提取、局部有效监督审计均 `通过`；方法效果仍为 `结论不足`，等待三组 MLP 对照。
...
849M 特征已确认无需重算，作为正式可复用大文件上传 OSS，不放 NAS。随后按固定公平协议运行全局轨迹、局部未补偿和局部已补偿三个轻量 MLP；核心验收仍是 aligned 相对 unaligned 的整体与 complex-motion AUC 增量及 case bootstrap 置信区间。
