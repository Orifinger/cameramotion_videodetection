...
| 2026-07-14 | 完整 DataB 检测模型的 Skyra 风格 GRPO 奖励动力学诊断 | 执行中；跳过 1 步门，直接运行论文式奖励 100 步 | 从既有检测 checkpoint 出发，比较论文式奖励及其问题消融如何改变正确率、真假偏置、证据计数和组内奖励方差 | 数据、verl 补丁、16 图像 vLLM 和真实 RLHFDataset 预检均通过；本实验用于学习训练曲线和奖励行为，不作为相机方法或泛化提升证据 |
