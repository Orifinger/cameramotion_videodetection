...
| 2026-07-12 | 相机补偿局部感知轨迹最小验证 | 分层 smoke 通过，待全量提取 | 在相同密集原视频帧和局部 mask 监督下，显式相机补偿是否稳定优于未补偿局部轨迹 | 9 个来源×运动组合的自动审计与三来源 Real/Fake 人工可视化均通过；尚未训练三组 MLP，不能建立检测提升 |
...
- 日期：2026-07-12。
- 状态：数据和权重预检、来源×运动分层 GPU smoke 自动审计及人工可视化均 `通过`；方法效果仍为 `结论不足`，等待全量提取和三组 MLP 对照。
...
运行 16 卡全量特征提取并执行全量审计；逐 case 大特征写入 `/tmp`，正式 manifest、audit 和后续探针结果写入 NAS 项目目录。全量审计通过后先备份需要复用的大特征到 OSS，再训练和比较三组 MLP。
