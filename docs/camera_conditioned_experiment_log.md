...
| 2026-07-15 | 正确相机二元前置强化学习与分阶段检测恢复 | 已提交执行，待服务器结果 | 从已经通过视觉依赖门的正确相机联合 SFT 模型出发，短程 Camera-PPRL 是否能在无相机文本推理时改善 ViF-Bench；随后检测回放能否恢复检测且保留相机能力 | 固定 1024 条平衡二元相机数据、1 epoch、每题 8 次采样；直接 PPRL 与恢复后结果分开保存，消融放到候选出现后 |
...
- Camera-PPRL：LoRA rank 32、alpha 64、学习率 `1e-6`、1 epoch、每题 8 次 rollout、temperature 0.7、beta 0.04；正确答案奖励 0.9、严格短格式奖励 0.1；冻结视觉塔和多模态 projector。
...
- 恢复分支使用同一 ViF 标准，相机 Macro AP 允许最多下降 5 点；即使通过也不能在缺少等计算控制时归因于相机 PPRL。
