...
| 2026-07-14 | 完整 DataB 检测模型的 Skyra 风格 GRPO 奖励动力学诊断 | 执行中；跳过 1 步门，直接运行论文式奖励 40 步 | 从既有检测 checkpoint 出发，比较论文式奖励及其问题消融如何改变正确率、真假偏置、证据计数和组内奖励方差 | 数据、verl 补丁、16 图像 vLLM 和真实 RLHFDataset 预检均通过；本实验用于学习训练曲线和奖励行为，不作为相机方法或泛化提升证据 |
...
- 日期：2026-07-14。
- 状态：`执行中；数据、verl 补丁、16 图像 vLLM 和真实 RLHFDataset 预检均通过，用户明确跳过 1 步门并直接启动论文式奖励 40 步短程诊断`。
...
2026-07-14 执行变更：DataB 构建、verl 三项兼容补丁、16 图像 vLLM smoke 和真实 `RLHFDataset` 预检均已通过。用户因无人值守安排明确选择跳过 1 步 optimizer smoke，直接运行 `paper_asymmetric_inspection` 的 40 步短程诊断。该变更增加了首步训练失败后浪费整晚算力的风险，但不改变数据、奖励、seed 或训练参数。

立即下一步：等待 40 步运行完成，保留每步 TensorBoard、训练日志、奖励分量、组内零方差率和逐样本 rollout；训练结束后自动上传 rollout 到 OSS，再执行 `/input/training/keep.sh`。次日先审计曲线与 reward hacking 指标，不立即把同一配置延长到完整 epoch。
