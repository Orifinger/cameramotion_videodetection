...
| 2026-07-14 | 完整 DataB 检测模型的 Skyra 风格 GRPO 奖励动力学诊断 | 已完成；训练链路通过，检测效果结论不足 | 从既有检测 checkpoint 出发，比较论文式奖励及其问题消融如何改变正确率、真假偏置、证据计数和组内奖励方差 | 100 步数值稳定；rollout 审计确认错误答案未获得正的组内相对优势，但仅 14.00% prompt 组提供分类差异、33.44% 只优化证据计数；未保存 checkpoint，不能判断固定评测是否提升 |
