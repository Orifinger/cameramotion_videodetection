...
| 2026-07-20 | 连续相机条件下的时序正常性专家验证（CTNE Gate 1） | 代码已就绪，待服务器执行 | 在不训练 Qwen、不输入 camera 文本的条件下，匹配的连续相机几何是否使真实视频时序正常性模型稳定优于同容量无条件与打乱条件 | 已完成可变帧 manifest、16-GPU RAFT/DINO 提取、真实视频条件 flow、三项核心控制、camera-only 捷径审计、双外部 benchmark 与三 seed 验收代码；尚无正式结果 |
