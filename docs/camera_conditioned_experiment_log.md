...
| 2026-07-13 | 同 16 帧二元相机辅助与检测回放联合训练三分支验证 | 工程 smoke 通过，正式训练待执行 | 在相同 16 帧输入和检测回放下，正确二元相机监督是否比逐条翻转监督及仅检测对照学到视觉相关能力，同时保留检测接口 | 正式三个等量分支完成注册；2/2 步 LoRA smoke 无 OOM 或非有限 loss，已具备正式训练条件，尚无模型效果结果 |
...
立即下一步：运行正确相机、翻转相机和仅检测回放三个 5-epoch 正式分支。正式 adapter 和 epoch checkpoints 是 `/tmp` 中的大型候选复用产物；先完成训练和相机/DataA 审计，确认会复用的分支再上传 OSS。先完成相机能力、翻转监督、视觉依赖和 `pass@8`，随后做 DataA 无相机文本检测保留；只有正确分支同时通过这些条件，才运行三分支 VIF-Bench 和短程 GRPO。
