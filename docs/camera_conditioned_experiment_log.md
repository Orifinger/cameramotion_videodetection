...
| 2026-07-13 | 有序抽帧二元相机辅助与检测回放联合训练三分支验证 | 相机能力子门通过；RL 门确认解析器不兼容，待重算；检测对照待训 | 在相同有序抽帧输入和检测回放下，正确二元相机监督是否比逐条翻转监督及仅检测对照学到视觉相关能力，同时保留检测接口 | 正确模型 matched Macro AP 86.28%、Balanced ACC 74.44%；7200 次 rollout 均为合法空 think 包装加 Yes/No，逐 rollout 语义正确率 76.50%，原 not-ready 是严格解析器误判，需用已有结果重算探索指标 |
...
- 日期：2026-07-13。
- 状态：`正确相机与翻转相机分支训练完成；双分支 held-out 相机门通过；仅检测回放对照待训练`。
