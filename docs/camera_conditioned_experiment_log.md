...
| 2026-07-13 | 有序抽帧二元相机辅助与检测回放联合训练三分支验证 | 三分支相机能力验收通过；RL 前置门 borderline；待 DataA 检测 | 在相同有序抽帧输入和检测回放下，正确二元相机监督是否比逐条翻转监督及仅检测对照学到视觉相关能力，同时保留检测接口 | 正确监督相对仅检测回放 Macro AP +14.48、Balanced ACC +12.44 点，相对翻转监督 +46.13/+44.22 点；视觉控制通过，建立相机能力注入，但尚未建立 AIGC 检测收益 |
...
- 日期：2026-07-13。
- 状态：`三个分支训练完成；三分支 held-out 相机能力验收通过；待无 camera 文本 DataA 检测评测`。
