...
| 2026-07-14 | 完整 DataB 检测模型的 Skyra 风格 GRPO 奖励动力学诊断 | tokenizer 审计通过，完整 VIF-Bench 待执行 | 从既有检测 checkpoint 出发，比较论文式奖励及其问题消融如何改变正确率、真假偏置、证据计数和组内奖励方差 | step 50/100 输入协议与 base 完全一致；按用户决定跳过 512 条诊断集，直接用完整 VIF-Bench 检验检测效果 |
