...
- 立即下一步：不运行 GenBuster Closed Benchmark，也不直接扩大训练。先仅用现有 Gate 2 items 做一次低成本分数尺度与决策边界诊断，区分失败来自固定 `beta=0.25` 的尺度不匹配，还是全局线性融合本身无法选择 157 个“仅专家正确”样本。
- 如果预先限定的全局线性诊断仍不能形成硬判定增益，则停止分数后融合，转向在训练阶段显式耦合时序 token 与 Real/Fake 目标；任何新方案均需重新登记独立验收标准。
