...
| 2026-07-14 | 完整 DataB 检测模型的 Skyra 风格 GRPO 奖励动力学诊断 | 第一轮完成；可评测复跑待执行 | 从既有检测 checkpoint 出发，比较论文式奖励及其问题消融如何改变正确率、真假偏置、证据计数和组内奖励方差 | 第一轮 100 步数值稳定但未保存模型；下一轮不调参，只保存 step 50/100 checkpoint，再做固定 DataB 策略漂移诊断和同协议 VIF-Bench |
