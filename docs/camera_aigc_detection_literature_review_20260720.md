...
Qwen3-VL 使用 interleaved-MRoPE 保留图像/视频 token 的时空位置关系，DeepStack 将多层 ViT 特征送入语言模型，并使用文字时间戳加强视频时间对齐。因此，16 帧有序输入并不是“模型看不见顺序”，也不能用 zero-shot 检测差直接断言模型完全没有视频能力。
...
- `C`：连续 camera context。第一版从已有 RAFT 全局流/鲁棒几何估计中提取每个 transition 的平移、旋转、尺度、shear、inlier ratio、拟合误差及其时间统计；CameraBench labels 只用于分层和语义审计。
- `Y`：需要被判定是否正常的时序取证表征。第一版复用 DINOv2 帧特征轨迹、一步/二步差分、曲率、自相似矩阵统计和未补偿 RAFT motion 统计。
- `Z`：最终 `Real/Fake` 标签。
...
使用 conditional normalizing flow 得到可计算的条件负对数似然：

```text
s_cam(X) = -log p_theta(Y(X) | C(X), Real)
```
...
1. 训练只使用 DataB/GenBuster 训练部分中的真实视频；按视频身份和真实来源拆分，不能按帧随机拆分。
2. `Y`、`C` 的 scaler、PCA 和 flow 只在训练 fold 拟合。
3. 阈值只在 held-out training-domain validation 选择；ViF-Bench 和 GenBuster benchmark 不调阈值。
4. DataA 不进入主训练；只在 Gate 1 通过后用于局部编辑/解释诊断。
...
- 使用 paired bootstrap CI、固定 prompt hash、相同 16 帧、相同 preprocessing；逐 generator 报告，不只给总体均值。
...
1. **先完成 Gate 1。** 复用已存在的 DINOv2、RAFT 和 16 帧数据，不下载新大模型，不训练 Qwen。
