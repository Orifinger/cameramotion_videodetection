...
- 固定 dev 身份：`tools/data/camera_motion_splits/dataA_test.json`。最终 40step_v3 的 train 严格取 1080 个完整 case 减去固定 321 个 dev，即 759 个；旧 `dataA_train.json` 只覆盖 746 个旧身份，默认不再读取。
